...
from src.paths import RESULTS_ROOT, INTERMEDIARY_ROOT, ensure_dir
...
def load_cem_vectors(experiment_name,concept_number,seed=-1,dataset_location=None):
    ...
    if dataset_location is None:
        dataset_location = str(RESULTS_ROOT / "bases" / "cem")
    elif not Path(dataset_location).is_absolute():
        dataset_path = Path(dataset_location)
        if dataset_path.parts and dataset_path.parts[0] == "results":
            dataset_location = str(RESULTS_ROOT.joinpath(*dataset_path.parts[1:]))
        else:
            dataset_location = str(RESULTS_ROOT / dataset_path)
...
def load_tcav_vectors(concept,bottlenecks,experiment_name="unfiled",seed=-1,alphas=[0.1],suffix=''):
    ...
    if seed == -1:
        seed = get_seed_numbers(str(RESULTS_ROOT / "bases" / "tcav" / experiment_name))
        if len(seed) == 0:
            raise Exception("No CAVs found at {}".format(RESULTS_ROOT / "bases" / "tcav" / experiment_name))
        seed = random.choice(seed)
    
    dataset_location = str(RESULTS_ROOT / "bases" / "tcav" / experiment_name / str(seed))
    all_matching_files = [] 
    concept_meta_info = []
...
def load_concept2vec_vectors_simple(attribute,dataset,suffix,seed=-1):
    ...
    dataset_location = str(RESULTS_ROOT / "bases" / "concept2vec")
...
def load_vector_from_folder(folder_name):
    def load_vectors_simple(attribute,dataset,suffix,seed=-1):
        return np.load(open(RESULTS_ROOT / "bases" / folder_name / (dataset.experiment_name+suffix) / str(seed) / "{}.npy".format(attribute),"rb"),allow_pickle=True)
...
def export_concept_vector(embedding_method,name,dataset,seeds):
    ...
    for seed in seeds:
        ...
        output_dir = ensure_dir(INTERMEDIARY_ROOT / "intervention_hierarchies" / dataset.root_folder_name)
        np.save(open(output_dir / "{}_{}.npy".format(name,seed),"wb"),all_embeddings)
