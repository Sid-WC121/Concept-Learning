...
def get_model_concept_similarities(dataset,model):
    ...
    img_paths_valid = [str(DATASET_ROOT / i['img_path']) for i in data_valid]
...
def truthfulness_metric_shapley(embedding_method,dataset,attributes,random_seeds,model_name="VGG16",baseline_hierarchies=None,compare_concepts=None):
    ...
    truthfulness_dir = ensure_dir(INTERMEDIARY_ROOT / "truthfulness")
    truthfulness_file = truthfulness_dir / "{}.npy".format(dataset.experiment_name)
    if os.path.exists(truthfulness_file):
        similarity_matrix = np.load(open(truthfulness_file,"rb"))

    else:
        model = get_large_image_model(dataset,model_name)
        model.load_weights(str(RESULTS_ROOT / "models" / "{}_models".format(model_name.lower()) / "{}_42.h5".format(dataset.experiment_name)))

        similarity_matrix = get_model_concept_similarities(dataset,model)
        np.save(open(truthfulness_file,"wb"),similarity_matrix)
