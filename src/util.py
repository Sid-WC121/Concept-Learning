...
def resave_models():
    ...
    ensure_dir(DATASET_ROOT / "models" / "keras")
    save_vgg_model(lambda w: w, str(DATASET_ROOT / "models" / "keras" / "model_vgg16.h5"))
    save_vgg_model(perturb_weights, str(DATASET_ROOT / "models" / "keras" / "model_vgg16_robust.h5"))
    save_vgg_model(responsive_weights, str(DATASET_ROOT / "models" / "keras" / "model_vgg16_responsive.h5"))
...
def save_concept_vectors(method,dataset,seed,file_name):
    ...
    directory = ensure_dir(INTERMEDIARY_ROOT / "intervention_hierarchies" / dataset.root_folder_name)
    ...
    np.save(open(directory / "{}.npy".format(file_name),"wb"),all_vectors)
