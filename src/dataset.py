...
def resolve_dataset_path(path):
    if os.path.isabs(path):
        return path
    return os.path.join(dataset_folder, path)
    ...
    def get_data(self,seed=-1,suffix="",train=True):
        ...
        if not os.path.exists(file_name):
            raise FileNotFoundError(
                "Missing dataset file {}. Set CONCEPT_DATASET_DIR or run scripts/dataset_scripts.".format(file_name)
            )
...
class MNIST_Dataset(Dataset):
    def __init__(self):
        ...
        self.path_to_image = resolve_dataset_path
...
class CUB_Dataset(Dataset):
    def __init__(self):
        ...
        classes_path = dataset_folder+"/CUB/metadata/classes.txt"
        if not os.path.exists(classes_path):
            raise FileNotFoundError(
                "Missing CUB metadata {}. Run scripts/dataset_scripts/download_cub.py or set CONCEPT_DATASET_DIR.".format(classes_path)
            )
    ...
    def get_attributes(self):
        ...
        if not os.path.exists(attributes_file):
            raise FileNotFoundError(
                "Missing CUB attributes {}. Run scripts/dataset_scripts/download_cub.py or set CONCEPT_DATASET_DIR.".format(attributes_file)
            )
...
def write_image(idx,imgs):
    ...
    ensure_dir(dataset_folder+'/dsprites/images')
...
def write_dataset(orientations,npz_file, write_images=False):
    ...
    ensure_dir(dataset_folder+'/dsprites/images')
    ensure_dir(dataset_folder+'/dsprites/preprocessed')
...
def write_ten_dsprites(seed=17):
