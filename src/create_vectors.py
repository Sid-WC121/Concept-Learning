...
def create_concept2vec(dataset,suffix,seed=-1,
    ...
    destination_folder = str(RESULTS_ROOT / "bases" / "concept2vec" / (dataset.experiment_name+suffix) / str(seed))
...
def create_tcav_dataset(attribute_name,dataset,num_random_exp,
    ...
    Side Effects:
        Trains a set of concept vectors, stored at results/bases/tcav/experiment_name/seed
    """
...
def load_activations_model(experiment_name,max_examples,model_name,sess):
    ...
    activation_dir = str(RESULTS_ROOT / "activations")
    ...
    if model_name == "GoogleNet":
        GRAPH_PATH = str(DATASET_ROOT / "models" / "inception5h" / "tensorflow_inception_graph.pb")
        ...
        if model_name == "Resnet50":
            GRAPH_PATH = str(DATASET_ROOT / "models" / "keras" / "model_resnet.h5")
        LABEL_PATH = str(DATASET_ROOT / "models" / "inception5h" / "imagenet_comp_graph_label_strings.txt")
        ...
        if model_name == "VGG16":
            GRAPH_PATH = str(DATASET_ROOT / "models" / "keras" / "model_vgg16.h5")
        elif model_name == "VGG16Robustness":
            GRAPH_PATH = str(DATASET_ROOT / "models" / "keras" / "model_vgg16_robust.h5")
...
def create_tcav_vectors(concepts,target,model_name,bottlenecks,num_random_exp,experiment_name="unfiled",alphas=[0.1],seed=-1,max_examples=100):
    ...
    Side Effects:
        Trains a set of concept_vectors stored at results/bases/tcav
    """
...
def create_model_vectors(attributes,dataset,suffix,seed=-1):
    ...
    output_dir = ensure_dir(RESULTS_ROOT / "bases" / "model_vectors" / (dataset.experiment_name+suffix) / str(seed))
        
    for attribute in activations:
        save_file = output_dir / "{}.npy".format(attribute)
...
def create_shapley_vectors(attributes,dataset,suffix,seed=-1,model_name="VGG16",noise_std=0,label_flip=0):
    ...
    img_paths = [str(DATASET_ROOT / i['img_path']) for i in data]
    ...
    if noise_std != 0:
        assert suffix == ""
        model.load_weights(str(RESULTS_ROOT / "models" / "{}_models".format(model_name.lower()) / "{}_noise_{}_{}.h5".format(
            dataset.experiment_name+suffix,noise_std,seed)))
    else:
        model.load_weights(str(RESULTS_ROOT / "models" / "{}_models".format(model_name.lower()) / "{}_{}.h5".format(
            dataset.experiment_name+suffix,seed)))
    
        
    if noise_std != 0:
        directory = str(RESULTS_ROOT / "bases" / "shapley" / "{}_noise_{}".format(dataset.experiment_name+suffix,noise_std) / str(seed))
    elif label_flip != 0:
        directory = str(RESULTS_ROOT / "bases" / "shapley" / "{}_flip_{}".format(dataset.experiment_name+suffix,label_flip) / str(seed))
    elif model_name != "VGG16":
        directory = str(RESULTS_ROOT / "bases" / "shapley" / "{}_model_{}".format(dataset.experiment_name+suffix,model_name.lower()) / str(seed))
    else:
        directory = str(RESULTS_ROOT / "bases" / "shapley" / (dataset.experiment_name+suffix) / str(seed))
