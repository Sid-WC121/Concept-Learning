...
def unzip_tar(file_location,folder_location):
    ...
    folder_location = ensure_dir(folder_location)
    for child in Path(folder_location).iterdir():
        if child.is_file():
            child.unlink()
    with tarfile.open(file_location) as tar:
        tar.extractall(folder_location)
...
def keep_n_files(folder_location,num_files):
    ...
    files = sorted([p for p in Path(folder_location).iterdir() if p.is_file()])
    for path in files[int(num_files):]:
        path.unlink()
...
def download_imagenet(imagenet_classes,images_per_class):
    ...
    imagenet_dataframe = fetcher.make_imagenet_dataframe(str(DATASET_ROOT / "meta" / "imagenet_url_map.csv"))
    source_dir = str(ensure_dir(DATASET_ROOT / "images"))
