...
def train_VAE(dataset,suffix,seed,save_location="", latent_dim=2,epochs=30,concept_alignment=False):
    ...
    if save_location != "":
        ...
        vae.save_weights(str(output_path))
...
def save_vae(model,dataset,suffix,seed,concept_alignment=False):
    ...
    folder_name = RESULTS_ROOT / "bases" / "vae" / (dataset.experiment_name+suffix) / str(seed)

    if concept_alignment:
        folder_name = RESULTS_ROOT / "bases" / "vae_concept" / (dataset.experiment_name+suffix) / str(seed)
    
    ensure_dir(folder_name)
        
    for concept in average_embedding_by_concept:
        file_name = folder_name / "{}.npy".format(concept)
...
def train_large_image_model(dataset,model_name,suffix="",seed=42,noise_std=0):
    ...
    if noise_std != 0:
        output_path = RESULTS_ROOT / "models" / "{}_models".format(model_name.lower()) / "{}_noise_{}_{}.h5".format(
            dataset.experiment_name+suffix,noise_std,seed)
    else:
        output_path = RESULTS_ROOT / "models" / "{}_models".format(model_name.lower()) / "{}_{}.h5".format(
