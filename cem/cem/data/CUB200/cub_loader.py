...
# General
BASE_DIR = ''
DATASET_ROOT = os.path.abspath(os.environ.get("CONCEPT_DATASET_DIR", os.path.join(os.getcwd(), "dataset")))
N_ATTRIBUTES = 112
N_CLASSES = 200
...
class CUBDataset(Dataset):
    ...
    def __init__(self, pkl_file_paths, use_attr, no_img, uncertain_label, image_dir, n_class_attr, root_dir='../data/CUB200/', transform=None, concept_transform=None,path_transform=None,correlation_rate=-1.0):
        ...
        for i in range(10):
            all_files = glob.glob(os.path.join(DATASET_ROOT, "colored_mnist", "images", str(i), "*.png"))
...
def load_data(
    ...
    if 'mnist' in pkl_paths[0] or 'dsprites' in pkl_paths[0]:
        transform = transforms.Compose([
            transforms.ToTensor(), #implicitly divides by 255
            ])
    elif 'chexpert' in pkl_paths[0]:
        transform = transforms.Compose([
