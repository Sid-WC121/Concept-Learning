...
LOGITS_ROOT = Path(os.environ.get("CONCEPT_LOGITS_DIR", REPO_ROOT / "results" / "logits")).resolve()

PATH_TO_LOGITS = str(LOGITS_ROOT / "{}" / "{}_c{}.npy")
...
def load_dataset(folder_name,fixed):
    ...
    if not folder_path.is_absolute():
        folder_path = DATASET_ROOT / folder_path
    logits_name = folder_path.name

    train = pickle.load(open(folder_path / "preprocessed" / "train.pkl","rb"))
    val = pickle.load(open(folder_path / "preprocessed" / "val.pkl","rb"))
    test = pickle.load(open(folder_path / "preprocessed" / "test.pkl","rb"))
    ...
    x_train = [np.zeros((224,224,3)) for i in range(train_size)]
    y_train = [train[i]['class_label'] for i in range(train_size)]
    c_train = np.load(open(PATH_TO_LOGITS.format(logits_name,"train",suffix),"rb"))

    x_val = [np.zeros((224,224,3)) for i in range(val_size)]
    y_val = [val[i]['class_label'] for i in range(val_size)]
    c_val = np.load(open(PATH_TO_LOGITS.format(logits_name,"valid",suffix),"rb"))
    
    x_test = [np.zeros((224,224,3)) for i in range(test_size)]
    y_test = [test[i]['class_label'] for i in range(test_size)]
    c_test = np.load(open(PATH_TO_LOGITS.format(logits_name,"test",suffix),"rb"))
