        ...
        # Ours: Intervene with related concepts
            ...
            if any(key in intervention_idxs for key in self.related_concepts[concept_idx].keys()):
                ...
                for reference_concept in self.related_concepts[concept_idx]:
                    if reference_concept not in intervention_idxs:
                        continue
                ...
                used_count = sum(1 for key in self.related_concepts[concept_idx].keys() if key in intervention_idxs)
                full_prob /= max(total_confidence, 1e-8)
                confidence = total_confidence / used_count
